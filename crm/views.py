...
def customer_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            customer.updated_date = timezone.localtime()
            customer.save()
            customer = Customer.objects.filter()
            return render(request, 'crm/customer_list.html',
                          {'customers': customer})
...
def product_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            product.updated_date = timezone.localtime()
            product.save()
            product = Product.objects.filter()
            return render(request, 'crm/product_list.html',
                          {'products': product})
...
def service_new(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            service.created_date = timezone.localtime()
            service.save()
            services = Service.objects.filter(created_date__lte=timezone.localtime())
...
def service_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            service = form.save()
            service.updated_date = timezone.localtime()
            service.save()
            services = Service.objects.filter(created_date__lte=timezone.localtime())
            return render(request, 'crm/service_list.html', {'services': services})
    else:
